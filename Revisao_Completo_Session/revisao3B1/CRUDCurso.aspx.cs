...
namespace revisao3B1
{
    public partial class CRUDCurso : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (Session["nomeUsuario"] == null) // verifica se a session foi iniciada
                    Response.Redirect("login.aspx");

                if ( !(Session["nomeUsuario"].ToString() == "admin" && Session["senhaUsuario"].ToString() == "123") ) //verifica o usuario e senha digitado
                    Response.Redirect("login.aspx");
